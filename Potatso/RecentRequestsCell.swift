...
class RecentRequestsCell: UITableViewCell {
    ...
    func config(request: Request) {
        ...
        if let e = request.events.first {
            timeLabel.text = RecentRequestsCell.dateformatter.string(from: NSDate(timeIntervalSince1970: e.timestamp) as Date)
        }else {
            timeLabel.text = nil
        }
    ...
    func setupLayout() {
        ...
        //timeLabel.setContentCompressionResistancePriority(UILayoutPriorityRequired, for: <#T##UILayoutConstraintAxis#>)
