...
class SyncVC: FormViewController {
        ...
        section
            <<< PushRow<SyncServiceType>() {
                ...
                if let type = row.value {
                    self?.showProgreeHUD()
                    SyncManager.shared.setupNewService(type: type, completion: { (error) in
                        self?.hideHUD()
                        if let error = error {
                            if let vc = self {
                                Alert.show(vc: vc, title: "Sync Service Setup Failed", message: "\(error)")
                            }
                        } else {
    ...
    func forceSync() {
        SyncManager.shared.sync(manually: true, completion: { (error) in
            if let error = error {
                Alert.show(vc: self, title: "Sync Failed", message:  "\(error)")
            } else {
                Alert.show(vc: self, title: "Sync Success")
            }
        })
    }
