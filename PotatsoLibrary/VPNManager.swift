...
public class PotatsoManager {
    
    public static let sharedManager = PotatsoManager()
...
extension ConfigurationGroup {
    ...
    public var isDefault: Bool {
        let defaultUUID = PotatsoManager.sharedManager.defaultConfigGroup.uuid
...
extension PotatsoManager {
    ...
    private func startVPNWithOptions(options: [String : NSObject]?, complete: ((NETunnelProviderManager?, Error?) -> Void)? = nil) {
        ...
        do {
            try PotatsoManager.sharedManager.regenerateConfigFiles()
        }catch {
            complete?(nil, error)
            return
        }
